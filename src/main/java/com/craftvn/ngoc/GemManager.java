...
public class GemManager {
    ...
    private final Map<UUID, Map<GemType, Long>> activeUntil = new HashMap<>(); // chống dồn cho SINH_LUC
    ...
    public void activate(Player p, GemType g){
        // Không cooldown cho tất cả ngọc.
        // Riêng SINH_LUC: không cho dùng dồn khi đang chạy.
        ...
        switch (g){
            case SINH_LUC -> {
                // +3 tim tức thì + Regen II trong 120s; xài 1 lần; không dồn
            ...
            case TRI_TRE -> {
                ...
                fx(p, Sound.BLOCK_HONEY_BLOCK_SLIDE, Particle.CLOUD);
            }
            case HO_VE -> {
                Player target = nearestPlayer(p, 10);
                // Spawn 3 Vex bảo vệ (không đánh chủ), tự retarget kẻ địch gần nhất, 20s tự biến mất
                for (int i=0;i<3;i++){
                    Vex v = (Vex) p.getWorld().spawnEntity(p.getLocation().add(0,1,0), EntityType.VEX);
                    v.setCustomName(cc("&cHộ vệ của "+p.getName()));
                    v.setCustomNameVisible(true);
                    // đánh dấu owner trong PDC
                    v.getPersistentDataContainer().set(NgocPlugin.KEY_GEM, PersistentDataType.STRING, "VEX_GUARD");
                    v.getPersistentDataContainer().set(new NamespacedKey(plugin, "owner"), PersistentDataType.STRING, p.getUniqueId().toString());
                    if (target != null) v.setTarget(target);

                    new BukkitRunnable(){
                        int ticks=0;
                        @Override public void run(){
                            if (v.isDead() || !v.isValid()){ cancel(); return; }
                            if (ticks >= 20*20){ v.remove(); cancel(); return; }
                            // tìm người chơi gần nhất khác chủ
                            Player t = v.getWorld().getNearbyPlayers(v.getLocation(), 12).stream()
                                    .filter(pl -> !pl.equals(p))
                                    .min(Comparator.comparingDouble(pl -> pl.getLocation().distance(v.getLocation())))
                                    .orElse(null);
                            if (t != null) v.setTarget(t);
                            ticks+=10;
                        }
            ...
            case LONG_KINH -> {
                // Lồng kính kiểu "mũ úp", không có sàn
            ...
            case TRUY_PHONG -> {
                Player target = p.getWorld().getNearbyPlayers(p.getLocation(),30).stream()
                        .filter(x->!x.equals(p) && (x.isGliding() ||
                                (x.getInventory().getChestplate()!=null && x.getInventory().getChestplate().getType()==Material.ELYTRA)))
