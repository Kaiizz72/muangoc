    ...
    public void activate(Player p, GemType g){
        ...
        switch (g){
            ...
            case HO_VE -> {
                ...
                // Spawn 3 Vex bảo vệ, KHÔNG đánh chủ
                for (int i=0;i<3;i++){
                    Vex v = (Vex) p.getWorld().spawnEntity(p.getLocation().add(0,1,0), EntityType.VEX);
                    v.setCustomName(cc("&cHộ vệ của "+p.getName()));
                    v.setCustomNameVisible(true);
                    // gắn owner vào PDC để listener biết chủ
                    v.getPersistentDataContainer().set(NgocPlugin.KEY_GEM, org.bukkit.persistence.PersistentDataType.STRING, "VEX_GUARD");
                    v.getPersistentDataContainer().set(new org.bukkit.NamespacedKey(plugin, "owner"), org.bukkit.persistence.PersistentDataType.STRING, p.getUniqueId().toString());
                    if (target != null) v.setTarget(target);
                    // retarget mỗi 10 tick tới kẻ địch gần nhất (không phải chủ)
                    new BukkitRunnable(){
                        int ticks=0;
                        @Override public void run(){
                            if (v.isDead() || !v.isValid()){ cancel(); return; }
                            if (ticks >= 20*20){ v.remove(); cancel(); return; } // ~20s
                            Player t = nearestEnemyExcluding(v.getLocation(), p);
